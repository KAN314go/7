...
class Spider(Spider):
    ...
    def searchContent(self, key, quick):
        result = {"list": []}
        try:
            search_url = f"{self.get_current_host()}/search.php?page=1&searchword={quote(key)}"
            ...
            # 修复：使用更健壮的正则来提取搜索结果
            pattern = r'<a class="link" href="(/post/[^"]+\.html)".*?<img[^>]*src="([^"]+)".*?alt="([^"]+)"'
            matches = re.findall(pattern, r.text, re.DOTALL)
            
            for match in matches[:15]:  # 限制15个结果
                try:
                    link, img_src, title = match
                    vod_id_match = re.search(r'/post/(.*?)\.html', link)
                    if not vod_id_match:
                        continue
