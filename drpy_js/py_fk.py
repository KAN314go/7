...
class Spider(Spider):
    ...
    def playerContent(self, flag, id, vipFlags):
        ...
        if m:
            ...
            if vurl.endswith('playlist.m3u8'):
                vurl = f"http://127.0.0.1:9978/proxy?do=py&type=m3u8&url={quote_plus(vurl)}"
        return {"parse": 0, "url": vurl, "header": self.header}


    def localProxy(self, params):
        if params['type'] == "m3u8":
            real_url = unquote(params['url'])
            print(">>> [M3U8] 请求地址:", real_url)
            try:
                headers = self.header.copy()
                headers.update({
                ...
                resp = self.fetch(real_url, headers=headers, timeout=10)
                content = resp.content.decode('utf-8', errors='replace')
                print(">>> [M3U8] 原始内容前300字符:\n", content[:300])
                
                # 🔥 关键：重写 .bgy 链接为代理链接
                def rewrite_bgy(match):
                    bgy_url = match.group(1)
                    proxy_url = f"http://127.0.0.1:9978/proxy?do=py&type=media&url={quote_plus(bgy_url)}"
                    return proxy_url
                
                content = re.sub(r'(https?://[^\s]+\.bgy)', rewrite_bgy, content)
                print(">>> [M3U8] 重写后内容:\n", content[:300])
    ...
    def proxyMedia(self, params):
        ...
        try:
            ...
            resp = self.fetch(url, headers=headers, stream=True)
            # ✅ 返回原始字节 + 正确 MIME（告诉播放器这是 TS 流）
            return [resp.status_code, "video/MP2T", resp.content]
        
        except Exception as e:
            print(">>> [MEDIA] 错误:", str(e))
